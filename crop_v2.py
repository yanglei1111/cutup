...
def slice_only_image(image_path, output_dir, slice_height, slice_width, overlap_height_ratio=0.2, overlap_width_ratio=0.2):
    # 确保输出目录存在
    ...
    for i, slice_bbox in enumerate(slice_bboxes):
        ...
        # 裁剪图像
        image_pil_slice = image_pil.crop((tlx, tly, brx, bry))

        # 保存切片图像
        slice_filename = f"slice_{i+1}.png"
        slice_filepath = Path(output_dir) / slice_filename
        image_pil_slice.save(slice_filepath)

        # 打印进度信息
        print(f"Saved sliced image: {slice_filepath}")

    # 关闭原始图像
    image_pil.close()
def slice_only_image_v2(image_path, output_dir, slice_height, slice_width, overlap_height_ratio=0.2, overlap_width_ratio=0.2):
    # 确保输出目录存在
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    ...
    for i, slice_bbox in enumerate(slice_bboxes):
        ...
        # 创建切片的文件名，包含坐标信息
        slice_filename = f"slice_{tlx}_{tly}_{brx}_{bry}.png"
        slice_filepath = Path(output_dir) / slice_filename

        # 保存切片图像
        cv2.imwrite(str(slice_filepath), image_slice)

        # 打印进度信息
        print(f"Saved sliced image: {slice_filepath}")
    return slice_bboxes
def slice_only_image_v3(image_path, output_dir, slice_height, slice_width, overlap_height_ratio=0.2, overlap_width_ratio=0.2):
    # 确保输出目录存在
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    ...
    for i, slice_bbox in enumerate(slice_bboxes):
        tlx, tly, brx, bry = slice_bbox
        image_slice = image_pil[tly:bry, tlx:brx]

        slice_filename = f"slice_{i}.png"
        slice_filepath = Path(output_dir) / slice_filename

        cv2.imwrite(str(slice_filepath), image_slice)  # 保存切片图像

        # 创建一个不包含COCO注释的CocoImage对象
        coco_image = CocoImage(file_name=slice_filename, height=image_slice.shape[0], width=image_slice.shape[1])
        sliced_image = SlicedImage(image=image_slice, coco_image=coco_image, starting_pixel=[tlx, tly])
...
if __name__ == "__main__":
    ...
    # 对图像进行切片
    #slice = slice_only_image_v2(image_path, output_dir, slice_height, slice_width)
    # slice_x = sorted(list(set([slice[i][0] for i in range(0, len(slice))])))
    # slice_y = sorted(list(set([slice[i][1] for i in range(0, len(slice))])))
    # from matplotlib import pyplot as plt
    # f, axarr = plt.subplots(len(slice_x), len(slice_y), figsize=(13, 13))
    # num = 0
    # for i in range(len(slice_x)):
    #     for j in range(len(slice_y)):
    #         img = Image.open(output_dir+"/slice_{}_{}_{}_{}.png".format(slice[num][0], slice[num][1], slice[num][2], slice[num][3]))
